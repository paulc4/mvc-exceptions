 ...
 * <ul>
 * <li><code>CONTROLLER_PROFILE</code> - creates a controller that also does its
 * own exception handling - see {@link ExceptionHandlingController}.
 * <li><code>GLOBAL_PROFILE</code> and <code>JAVA_CONFIG_PROFILE</code> -
 * creates a controller with no exception handlers. Instead exceptions are
 * handled globally - see {@link ControllerWithoutExceptionHandlers} and
 ...
 * <li><code>GLOBAL_PROFILE</code> and <code>XML_CONFIG_PROFILE</code> - as
 * previous but the {@link SimpleMappingExceptionResolver} is configured using
 * XML - see <code>mvc-configuration.xml</code>
 * </ul>
 * The ability to switch between Java or XML configuration for the
 * <code>SimpleMappingExceptionResolver</code> is just for demonstration
 * purposes. In a real application use one or the other, but not both. You are
 * very likely to have an existing XML definition for this bean and there is no
 * need to change it.
 *
 * @author Paul Chapman
 */
public interface Profiles {
	...
	/**
	 * DEMO mode configuration property - see {@link DemoExceptionConfiguration}.
	 * Value = <b>{@value} </b>
	 */
